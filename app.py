...
def seed_db():
    for i in range(1, 20):
        new_jet = Jet(id=i, side=randint(400, 700), parking=randint(1, 40), fuel=choice([True, False]),
                      dta=choice([True, False]), arm=choice([True, False]), flying=choice([True, False, False, False]),
                      status=choice([True, True, True, False]))
...
def seed_sked_db():
    ...
    for j in range(len(skedreader) + 10):
        if j < len(skedreader):
...
def get_settings():
    if not session.get('logged_in'):
        ...
        if request.method == 'POST':
            for k in ['rows', 'refresh', 'per_row', 'msg_lines','clockoffset']:
